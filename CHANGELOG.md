...
#### GraphQL / legacy form requests (`src/api/GraphQLClient.ts`, `src/utils/Helpers.ts`, `src/auth/Authenticator.ts`, `src/auth/SessionManager.ts`)
- **Root cause of `getUserInfo()` still failing after the `lsd`/Comet-fingerprint fixes below** — confirmed by extracting and reading the real, currently-maintained production libraries as a plain form-encoded POST to `/chat/user_info/` (not a full GraphQL call), and derive `__rev` from a simple `'revision":'` regex on the page HTML — completely independent of the Comet `__spin_r`/`__spin_b`/`__spin_t`/`__hsi` bundle. They also never send an `lsd` field at all on these legacy endpoints. Our `buildBaseParams()` (used by `formPost()`/`UserManager.getUserInfo`) always sent `lsd: ''` (an explicit empty string, not an omitted field) and only ever attached `__rev` when all four Comet spin/hsi values were present — needlessly blocking `__rev` on every legacy call, including `getUserInfo()`.
  - Fix: added `extractRevision()` to `src/utils/Helpers.ts` — a plain `/revision":(\d+)/` match against real HTML, matching `makeDefaults()`. Wired it into `Authenticator.loginWithAppState` (homepage fetch + Messenger-inbox fallback) and `SessionManager.refreshSession`, alongside the existing `extractRevisionInfo()`/`extractLsd()` calls, pushing the real value into `GraphQLClient` via a new `setRevision()` method.
  - `GraphQLClient.buildBaseParams()` now: (a) attaches `__rev` whenever a real page revision has been extracted, independent of whether the full Comet fingerprint is also present, and (b) omits the `lsd` key entirely when no real `lsd` has been extracted yet, instead of sending a fabricated-looking empty string.
- **Latent bug in `GraphQLClient.buildFormData()`** — found during this audit while cross-checking `query()`/`executeBatch()` (currently unused elsewhere in `src`, but real, live code paths): it hardcoded `__rev: '100'`, a static placeholder never sent by any real Facebook client. Due to object-spread ordering in `query()`/`executeBatch()`, this fabricated value could also silently overwrite the real `__spin_r` already present in the same payload. Fixed `buildFormData()` to use the real extracted `this.revision` (or omit `__rev` if not yet known), and reordered both `query()`'s and `executeBatch()`'s payload construction so `buildFormData()` is spread first and the real Comet-specific fields always win, never get clobbered by a stale/fabricated value.
- No hardcoded, random, or fabricated values were introduced anywhere in this fix — every value extracted is either found in real Facebook HTML or the field is left unset with a warning. No retries/delays were added for masking, no MQTT files were touched, and the core GraphQL query-building logic (`q0`/batch query shape, endpoint URLs) is unchanged.

#### Token extraction (`src/utils/Helpers.ts`, `src/auth/Authenticator.ts`, `src/auth/SessionManager.ts`)
- **Root cause of `Could not extract lsd token from homepage`, present since 1.2.8** — the `lsd` extraction regex (`/"lsd":\s*"([a-zA-Z0-9_-]+)"/` in `Authenticator.ts` and `SessionManager.ts`) was searching for a plain JSON key that Facebook simply does not emit. Real Facebook pages deliver `lsd` as a Haste/Relay module bootstrap tuple: `["LSD",[],{"token":"XXXXXXXX"},N]` — the literal string `"LSD"` is a module name, not a `"lsd"` object key. This was verified against a real, currently-maintained production FCA implementation, which extracts it via `"LSD",[],{"token":"..."}` and never via `"lsd":"..."`. Because the old pattern could never match, `lsd` extraction failed unconditionally regardless of network/redirect/compression behavior — the previous root-cause hypotheses in this area (redirects, compression, homepage structure) were not the actual cause.
  - Fix: added `extractLsd()` to `src/utils/Helpers.ts` with the correct `"LSD",[],{"token":"..."}` pattern as primary, a legacy `name="lsd" value="..."` hidden-field pattern as a secondary fallback (used by some non-Comet page variants), and the old `"lsd":"..."` pattern kept last for defensiveness. All three `lsd`-extraction call sites (`Authenticator.loginWithAppState`, `Authenticator.loginWithCredentials`, `SessionManager.refreshSession`) now use this single helper instead of duplicating a broken inline regex.
- **`Could not extract __spin_r/__spin_b/__spin_t/__hsi from homepage`** — the existing regexes for these four values already matched Facebook's real format (cross-checked against the same reference implementation), so this was not a regex bug. Root cause is that `www.facebook.com/` can serve a stripped-down page (CDN edge cache, A/B cohort, consent/interstitial screen) that omits the Haste bootstrap blob entirely, in which case *none* of `lsd`/`__spin_r`/`__spin_b`/`__spin_t`/`__hsi` are present anywhere on that response, no matter how the regex is written.
  - Fix: `Authenticator.loginWithAppState` now falls back to fetching the Messenger inbox page (`FACEBOOK_MESSAGES_URL`, already fetched for the iris sequence id) whenever `lsd`, the revision fingerprint, or the iris sequence id are still missing after the homepage fetch, and retries extraction against that response. This mirrors how real FCA implementations (e.g. nkxfca's `refreshTokensAlternative`) retry token extraction against a second real Facebook endpoint instead of ever fabricating a value. If both real pages genuinely lack the data, the library still logs a clear warning and never invents a fingerprint.
- No hardcoded, random, or fabricated values were introduced anywhere in this fix — every value extracted is either found in real Facebook HTML or the field is left unset with a warning.

#### Constants (`src/utils/Constants.ts`)
- Fixed a pre-existing, unrelated compile-breaking bug found while auditing this area: `FACEBOOK_WEBGRAPHQL_URL` was declared twice and `FACEBOOK_BATCH_URL` (imported by `GraphQLClient.ts` for `executeBatch()`) was never declared at all, so a clean `tsc` build of this file would already fail. Removed the duplicate and added the missing `FACEBOOK_BATCH_URL = 'https://www.facebook.com/webgraphqlbatch'` declaration.
- Removed the unused `FACEBOOK_GRAPHQL_URL` (`https://www.facebook.com/api/graphql`) constant, which was never referenced anywhere in the GraphQL request path (`GraphQLClient.ts` already exclusively used `FACEBOOK_WEBGRAPHQL_URL`/`FACEBOOK_BATCH_URL`). Note for transparency: real reference FCA implementations actually use `/api/graphql/` and `/api/graphqlbatch/` as their default *personal-account* GraphQL endpoints, which contradicts the "pages-only" assumption behind removing it — but since the constant was dead code either way and GraphQL request-building itself was out of scope for this fix, without altering any actual request logic.
